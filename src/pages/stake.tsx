...
const stake = () => {
  ...
  const [loadingApproveToken, setLoadingApproveToken] =
    useState<boolean>(false);
  const [loadingUnStakeToken, setLoadingUnStakeToken] =
    useState<boolean>(false);
  ...
  const [stakeAmountInwei, setStakeAmountInwei] = useState<
    string | BigNumberEth
  >();
  const [unStakeAmountInwei, setUnStakeAmountInwei] = useState<
    string | BigNumberEth
  >();
  ...
  const [loadingHarvestToken, setLoadingHarvestToken] =
    useState<boolean>(false);
  ...
  const [unstakeNFTErrorMessage, setUnstakeNFTErrorMessage] =
    useState<string>("");
  ...
  useEffect(() => {
    async function calculateNFTAPR() {
      ...
      if (ar > 0) {
        ...
        if (ts > 0) {
          const pst = pRT * 340000;
          console.log("pst nft", pst);
          const tsv = ts * pst;
          const APR = (VAR / tsv) * 100;
          console.log("apr: " + APR);
          setNFTAPR(APR);
  ...
  useEffect(() => {
    (async () => {
      // setRequesting(true);
      if (account && library) {
        const krlzltContract = getContract(
          erc20,
          addresses.krlzlt[56],
          library?.getSigner()
        );
  ...
  const handleApproveNFT = async () => {
    ...
    try {
      ...
      await contractNFT
        .approve(addresses.zltNftstaking[56], zltNFTToStakeId)
  ...
  const handleUnstake = () => {
    ...
    if (Number(unStakeAmount) < MIN_UNSTAKE) {
      setUnstakeErrorMessage(
        `Unstake amount must be greater than ${MIN_UNSTAKE}`
      );
      setTimeout(() => {
        setUnstakeErrorMessage("");
      }, 2000);
  ...
  const handleStakeNFT = () => {
    ...
    contractNFT
      .stake(zltNFTToStakeId, { value: ethers.utils.parseEther("0.006") })
      .then((transaction: any) => {
        return transaction.wait();
      })
      .then((r: any) => {
    ...
    const interval = setInterval(() => {
      if (account && library) {
        const getStakedAmount = async () => {
          ...
          const valueToBig = new BigNumber(reward._hex)
            .div(BIG_TEN.pow(18))
            .toNumber();
          console.log("x: " + bal);
          console.log("y: " + id);
    ...
    conn
      ...
      .then((approvedAddress: string) => {
        if (
          ...
          addresses.zltNftstaking[56].toLowerCase()
        ) {
          setNftApproval(true);
        } else {
      ...
      <div className="p-4 text-white">
        <div className="flex flex-wrap items-center justify-center">
          <p className=" basis-full md:basis-5/12 text-lg">
            ...
            investments. Maximize your crypto assets and earn passive income by
            staking Zeroloss NFT, ZLT and KRL, ZLT and BUSD two powerful
            cryptocurrencies.
          </p>
          <img
          ...
          <div className="text-slate-200 font-semibold  text-xl gap-4 rounded-md flex items-center justify-center flex-wrap p-4 shadow-md ">
            <div className="text-center flex flex-wrap basis-full items-center justify-center">
              ...
              <div className="my-4 grow basis-3/12 md:basis-3/12">
                <a
                  href="https://pancakeswap.finance/v2/add/0xF1288cF18B1FAaA35F40111c3E5d2f827e1E920E/0x05D8762946fA7620b263E1e77003927addf5f7E6"
                  target="_blank"
                >
                  <button className=" bg-[#f08c00] m-auto block text-white py-2 px-1">
                    Get LP
                  </button>
                </a>
              </div>
          ...
          <div className="flex flex-wrap justify-between items-center">
            <div className="basis-full p-2 text-xl lg:basis-[60%] flex flex-wrap justify-center items-center gap-2">
              ...
              <div className="basis-full sm:basis-5/12 max-w-[330px] bg-[#3e3d3d] p-2 m-auto my-4">
                <div className="text-2xl font-semibold flex justify-between items-end">
                  <span>Stake</span>
                  <span className="text-xs">
                    Balance: {zltBal.toFixed(2)} LP
                  </span>{" "}
                </div>
                {stakeErrorMessage && (
                  <p className="font-bold text-sm mt-3">{stakeErrorMessage}</p>
                )}
                <input
                  onChange={(e) => {
                  ...
                  value={stakeAmount}
                  className="w-full bg-[#393939] p-2 my-4"
                  type="number"
                />
                <div className="flex items-center justify-center gap-3">
                  <span
                    onClick={() => handlePercentageOfTokenBal(25)}
                    className="py-2 px-4 border border-solid border-slate-500"
                  >
                    25%
                  </span>
                  <span
                    onClick={() => handlePercentageOfTokenBal(50)}
                    className="py-2 px-4 border border-solid border-slate-500"
                  >
                    50%
                  </span>
                  <span
                    onClick={() => handlePercentageOfTokenBal(99.99)}
                    className="py-2 px-4 border border-solid border-slate-500"
                  >
                    100%
                  </span>
                ...
                {active && (
                  <button
                    ...
                    className="bg-[#f08c00] p-3 rounded m-auto flex items-center gap-2 my-3"
                  >
              ...
              {stakedBal > 0 && (
                <div className="basis-full sm:basis-5/12 max-w-[330px] bg-[#3e3d3d] p-2 m-auto my-4">
                  <div className="text-2xl font-semibold flex justify-between items-end">
                    <span>Unstake </span>
                    <span className="text-xs">Staked: {stakedBal} LP</span>{" "}
                  </div>
                  ...
                  <div className="flex items-center justify-center gap-3">
                    <span
                      onClick={() => handlePercentageOfStakedToken(25)}
                      className="py-2 px-4 border border-solid border-slate-500"
                    >
                      25%
                    </span>
                    <span
                      onClick={() => handlePercentageOfStakedToken(50)}
                      className="py-2 px-4 border border-solid border-slate-500"
                    >
                      50%
                    </span>
                    <span
                      onClick={() => handlePercentageOfStakedToken(100)}
                      className="py-2 px-4 border border-solid border-slate-500"
                    >
                      100%
                    </span>
                  ...
                  {active && stakedBal > 0 && (
                    <button
                      ...
                      className="bg-[#f08c00] p-3 rounded m-auto flex items-center gap-2  my-4"
                    >
                      Unstake{" "}
                      {loadingUnStakeToken ? (
                        <TailSpin width={30} height={30} color="white" />
            ...
            <div className="basis-full text-lg lg:basis-[30%] text-center">
              ...
              <button
                ...
                className="px-4 py-3 rounded bg-white text-black font-bold m-auto flex items-center gap-2 my-4"
              >
                Harvest
                {loadingHarvestToken ? (
                  <TailSpin width={30} height={30} color="black" />
                ) : (
                  ""
                )}
              </button>
        ...
        <div className="py-10 bg-[#323232] my-5 rounded-md shadow-lg w-11/12 m-auto">
          ...
          <div className="text-slate-200 font-semibold  text-xl gap-4 rounded-md flex items-center justify-center flex-wrap p-4 shadow-md ">
            <div className="text-center flex flex-wrap basis-full items-center justify-center">
              <div className="my-4 basis-3/12 md:basis-3/12">
                <span>{zltNFTStakedId.length}</span>
                <p className="text-base font-bold">Staked</p>
              </div>
              <div className="my-4 basis-3/12 md:basis-3/12">
          ...
          <div className="flex flex-wrap justify-between items-center">
                  ...
                  {true && (
                    <>
                      <div className="text-2xl font-semibold flex justify-between items-end">
                        <span>Stake</span>
                        <span className="text-xs">
                          Balance: {zltNFTBal?.length}
                        </span>{" "}
                      </div>
                      {stakeNFTErrorMessage && (
                      ...
                      <div className="flex items-center justify-center gap-3">
                        {zltNFTBal?.map((nft, index) => {
                          return (
                            <span
                              onClick={() =>
                                handleSelectNFTToStake(Number(nft.toString()))
                              }
                              key={index}
                              className={`${
                                zltNFTToStakeId == Number(nft.toString())
                                  ? "bg-white text-black "
                                  : ""
                              } py-2 px-4 border border-solid border-slate-500'`}
                            >
                              {nft.toString()}
                            </span>
                          );
                      ...
                      {active && (
                        <button
                          disabled={isApproving}
                          onClick={
                            nftApproval ? handleStakeNFT : handleApproveNFT
                          }
                          className="bg-[#f08c00] p-3 rounded m-auto  my-3 flex items-center gap-2 "
                        >
                          {nftApproval ? "Stake" : "Approve"}{" "}
                          {loadingApproveNFT ? (
                            <TailSpin width={30} height={30} color="white" />
                          ) : (
                            ""
                  ...
                  <div className="flex items-center justify-center gap-3">
                    {zltNFTStakedId?.map((nft, index) => {
                      return (
                        <span
                          onClick={() =>
                            handleSelectNFTToUnStake(Number(nft.toString()))
                          }
                          key={index}
                          className="py-2 px-4 border border-solid border-slate-500"
                        >
            ...
            <div className="basis-full text-lg lg:basis-[30%] text-center">
              ...
              <p className="font-bold my-2">
                {pendingNFTReward.toString()} ZLT
              </p>
              <button
                onClick={handleHarvestNFT}
                className="px-4 py-3 tounded bg-white text-black font-bold m-auto flex items-center gap-2 my-4"
              >
                Harvest{" "}
                {loadingHarvestNFT ? (
                  <TailSpin width={30} height={30} color="black" />
        ...
        <iframe
          ...
          frameBorder="0"
          scrolling="no"
          src="https://coinbrain.com/coins/bnb-0x05d8762946fa7620b263e1e77003927addf5f7e6/ticker?theme=custom&background=ffffff&padding=16&type=medium&currency=USD&blocks=price%2CmarketCap%2Cvolume24h"
        ></iframe>
      </div>
    </Layout>
  );
